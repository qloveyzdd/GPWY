...
function buildChartOptions(snapshot: ChartSnapshot): EChartsOption {
  ...
  if (
    snapshot.overlays.chipPeakState === "available" &&
    snapshot.overlays.chipPeakPrice !== null
  ) {
    markLineData.push({
      name: "筹码峰",
      yAxis: snapshot.overlays.chipPeakPrice,
      lineStyle: { color: "#15803D", type: "solid" as const },
      label: { formatter: "筹码峰" },
    });
  }
      ...
      <div className="mb-3 flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Badge variant="outline">
            ...
            {snapshot.overlays.chipPeakPrice === null
              ? "无数据"
              : formatPrice(snapshot.overlays.chipPeakPrice)}
          </Badge>
        </div>
      </div>
      <div
