...
- [ ] **DATA-11**: 系统可以获取并缓存种子日到目标日之间的日线、成交量、换手率和复权因子，保证筹码推演使用同一复权口径。
- [ ] **DATA-12**: 系统将官方原始 `cyq_chips`、60 日种子分布和推演后的计算分布分开存储，不用计算结果覆盖官方数据。

### Decay-Based Chip Calculation

- [ ] **CMOD-01**: 系统以 60 日前种子筹码分布为初始状态，按后续每个交易日的交易数据逐日转换为目标日期的计算筹码分布。
- [ ] **CMOD-02**: 每个交易日的旧筹码衰减和新增筹码分配必须使用可解释公式，至少纳入换手率、成交价格区间和复权调整，不使用无法追溯的纯数值增减。
- [ ] **CMOD-03**: 用户可以在固定衰减系数集合中选择一个值：`0.3 / 0.5 / 0.8 / 1.0 / 1.2 / 1.5 / 2.0`。
- [ ] **CMOD-04**: 同一股票、目标日期和衰减系数组合的计算结果可以被复用；当种子分布或交易数据缺失时，系统明确标记不可用原因。
...
- [ ] **VAL-01**: 算法单元测试覆盖衰减系数、复权调整、缺失交易数据、缺失种子分布和双目标日期计算。
