...
下一里程碑的当前需求将在运行 `$gsd-new-milestone` 时重新创建。
...
- **Validated:** 37/37 v1 requirements completed and mapped to passed phase verification.
- **Adjusted:** `SCRN-04` 和 `SCRN-05` 从原始局部高点/兜底定义调整为用户确认的连续更高最高价向前回溯规则。
- **Verified late:** `REFR-05` 的缓存覆盖在里程碑审计阶段补齐正式验证证据。
- **Dropped:** None.
- **Deferred:** 自动刷新、历史对比、可配置参数、CSV 导出、多人权限、PostgreSQL 和后台队列保留为后续候选，不属于 v1.0。
