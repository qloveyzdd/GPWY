...
v1.0 交付了可云端自托管的个人 A 股下降趋势筛选工具：从服务端数据源验证与手动刷新开始，经过 SQLite 缓存和可解释筛选，再到官方筹码峰、结果表格、行内 K 线图和部署验证。

---

# Archived Roadmap: A Stock Downtrend Screener
...
- 使用个人访问密码和服务端环境变量保护工作台与授权信息。
- 使用 SQLite 保存刷新、筛选和筹码结果的任务级成功快照。
- 使用 Tushare 作为主数据源，tinyshare 作为显式兼容 provider。
- 区间高点按最新交易日起向前连续更高最高价回溯，停止条件为前一日最高价不再更高。
- 筹码峰只使用官方 `cyq_chips`，按占比展示前三价格档，不使用估算替代。

**Issues Resolved:**

- 修复 tinyshare 中文股票名称编码问题。
- 对齐用户确认的区间高点规则及回归测试。
- 补齐 Phase 2 阶段验证和所有阶段 Nyquist 验证映射。
- 将图表改为点击股票行后在对应行下方按需显示。

**Issues Deferred:**

- 自动每日刷新和历史结果对比。
- 可配置筛选参数与 CSV 导出。
- 多人权限、PostgreSQL 和后台任务队列。

**Technical Debt Incurred:**

- 官方筹码 enrichment 当前串行执行，入选股票较多时刷新耗时会增长。
