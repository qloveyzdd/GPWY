...
用户可以可靠地筛出当前价格低于最近下降区间波段高点 85% 的 A 股，并直观看到最新有效交易日与前一有效交易日的官方完整筹码分布和可解释计算筹码分布。

## Current Milestone: v2.1 衰减筹码分布模型

**Goal:** 获取目标日前 60 个交易日的官方筹码分布作为种子，再按后续交易、换手和可选衰减系数推演目标日计算筹码分布。

**Status:** Phase 12 verified as of 2026-07-01; ready for v2.1 milestone close-out.
...
- ✓ 获取目标日前 60 个交易日的官方 `cyq_chips` 种子分布 — Phase 12
- ✓ 按种子日至目标日之间的交易、换手和复权数据逐日推演计算筹码分布 — Phase 12
- ✓ 支持衰减系数 `0.3 / 0.5 / 0.8 / 1.0 / 1.2 / 1.5 / 2.0` — Phase 12
- ✓ 将官方原始分布、种子分布和计算分布分开缓存与展示 — Phase 12
- ✓ 在股票详情页切换并标注计算筹码分布，不把计算结果伪装成官方 `cyq_chips` — Phase 12
...
None — v2.1 Phase 12 is verified and awaiting milestone close-out.
...
- v2.1 Phase 12 已于 2026-07-01 通过验证，交付以 60 日前官方筹码分布为种子的衰减推演模型、缓存和详情页切换展示。
- 当前技术栈为 Next.js 16、React 19、TypeScript、SQLite、ECharts、Vitest 和 Playwright。
- 完整验证为 33 个测试文件、213 项测试及 1 项浏览器冒烟测试。
...
| 计算筹码分布必须独立于官方 `cyq_chips` | 衰减模型是可解释估算，不是官方数据源；必须避免把模型误认为事实数据 | ✓ Phase 12 验证 |

## Current State

**Shipped version:** v2.0 — 2026-06-30

**Active milestone:** v2.1 衰减筹码分布模型 — Phase 12 verified, ready for close-out.
...
- 工作区支持登录、数据源验证、可恢复增量刷新、阶段进度、筛选、双日官方筹码分布、计算筹码分布系数切换、排序和行内图表。
- 云端部署所需环境变量、PM2 启动方式和验证命令已记录在 README。

## Next Milestone Goals

待用户确认下一轮需求。当前可选方向是继续校准筹码分布模型、补充真实样本对比，或进入其他筛选/展示增强。
...
---
*Last updated: 2026-07-01 after Phase 12 verification*
