...
**PASSED.** Phase 12 achieved its roadmap goal: the system can use a 60-trading-day-prior official seed distribution, transform it with daily trading data and fixed decay coefficients, cache calculated results separately, and display them in stock detail with explicit model labeling.
...
| DATA-12 | Passed | `chip_distribution_*`, `chip_model_seed_snapshots`, and `chip_model_levels` are separate tables; store tests assert calculated data does not overwrite official distribution data. |
...
| CMOD-04 | Passed | Cache keys include stock, target date, seed date, coefficient, and model version; unavailable statuses keep reason/error fields. |
...
- Official distributions and calculated distributions remain separate in storage, DTO, and UI.
- Default detail view uses coefficient 0.5.
- Selector allows only the fixed coefficient set.
- Coefficient switching does not invoke a calculation endpoint.
