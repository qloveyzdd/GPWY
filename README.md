...
5. 在“计算分布”区域切换衰减系数 `0.3 / 0.5 / 0.8 / 1 / 1.2 / 1.5 / 2`，对比模型输出的 latest/previous 计算筹码分布。

筹码接口无权限、空数据或临时失败时，股票仍保留在筛选结果中；对应交易日的筹码分布卡片会显示不可用状态和脱敏原因，另一交易日可用时仍正常展示。计算分布独立标注为“模型输出，不等同官方 cyq_chips”，不会覆盖官方数据。
...
- 计算筹码分布以 60 个交易日前的官方筹码分布为种子，结合后续交易数据、换手率和复权因子逐日推演；官方原始分布、种子快照和计算分布分表保存。
- 股票详情页可在固定衰减系数 `0.3 / 0.5 / 0.8 / 1 / 1.2 / 1.5 / 2` 中切换，默认展示 `0.5`，切换只读取缓存结果，不触发即时计算。
