...
**当前版本：v2.0 已收尾归档（2026-06-30）；v2.1 需求已定义**

v1.0 已完成 37 项需求和浏览器冒烟验证。v2.0 已完成 27 项需求：标准化行情缓存、统一 provider 调度、持久 tinyshare worker、可恢复增量刷新、双日完整筹码分布缓存和双图对比体验。v2.1 规划方向是以 60 个交易日前的官方筹码分布为种子，按后续交易和可选衰减系数推演目标日计算筹码分布。里程碑规划记录位于 `.planning/`。
...
3. 查看“最新筛选结果”表格。
4. 点击表格行，在下方查看该股票 60 个交易日 K 线图，以及最新有效交易日和前一有效交易日的完整筹码分布图。

筹码接口无权限、空数据或临时失败时，股票仍保留在筛选结果中；对应交易日的筹码分布卡片会显示不可用状态和脱敏原因，另一交易日可用时仍正常展示。当前已实现的 v2.0 图表不会使用估算值替代官方筹码数据；v2.1 计划引入的计算分布会独立标注，不覆盖官方数据。
...
- 官方筹码分布覆盖最新有效交易日和前一有效交易日，保存完整价格档位和占比；不可用时记录交易日级状态和脱敏原因，不做估算。
- 结果表格支持关键趋势指标排序，不再展示筹码峰字段或筹码峰排序。
- 内联详情展示 K 线、MA20、MA60、区间高点、85% 阈值，以及最新/前一有效交易日两张完整筹码分布图。
