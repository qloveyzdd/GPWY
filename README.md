...
v1.0 已完成 37 项需求和浏览器冒烟验证。v2.0 已完成 27 项需求：标准化行情缓存、统一 provider 调度、持久 tinyshare worker、可恢复增量刷新、双日完整筹码分布缓存和双图对比体验。v2.1 Phase 12 已完成 13 项需求：以 60 个交易日前的官方筹码分布为种子，先做重分桶和平滑扩散，再按后续交易、换手率和复权因子推演目标日计算筹码分布，并支持固定衰减系数切换。里程碑规划记录位于 `.planning/`。
...
- 计算筹码分布以 60 个交易日前的官方筹码分布为种子，复权调整后按 `0.05` 元重分桶并做 `±0.20` 元三角核平滑扩散，再结合后续交易数据、换手率和复权因子逐日推演；系统只持久保存官方原始分布和种子快照，目标日计算分布在详情页按需生成，不再全量预计算入库。
