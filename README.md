...
**当前版本：v2.0 实现完成（2026-06-30）**

v1.0 已完成 37 项需求和浏览器冒烟验证。v2.0 已完成 27 项需求：标准化行情缓存、统一 provider 调度、持久 tinyshare worker、可恢复增量刷新、双日完整筹码分布缓存和双图对比体验。里程碑规划记录位于 `.planning/`。
...
筹码接口无权限、空数据或临时失败时，股票仍保留在筛选结果中；对应交易日的筹码分布卡片会显示不可用状态和脱敏原因，另一交易日可用时仍正常展示。系统不会使用估算值替代官方筹码数据。
